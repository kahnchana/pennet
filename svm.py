...
def syntheticData(args):  # generate fake data
    # class 0
...
def buildDataLoader(path, args):  # load real data
...
def train(epoch, model, trn_loader, optimizer, loss, n_class, args):
    """training"""
    model.train()
    training_loss = 0
    training_f1 = 0
...
def test(model, tst_loader, y_test_list, args):
    """testing"""
    model.eval()
    test_loss = 0
    preds = list()
...
def main():
    ...
    trn_loader, tst_loader, n_feature, n_class, y_train_list, y_test_list = syntheticData(args)
